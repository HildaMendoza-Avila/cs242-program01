...
ui <- fluidPage(
  checkboxGroupInput("energySourceChoice", "Choose an energy source(s):", energySourceList, selected="all"), 
  plotOutput("sourceSelection"),
  plotOutput("sourceSelection"),
  plotOutput("sourceSelection"),
  plotOutput("sourceSelection")
...
server <- function(input, output, session) {
  ...
  output$sourceSelection <- renderPlot({
    ggplot(usTotalEnergySources, aes(fill=EnergySource, x=Year, y=MegawattHours, width=0.8)) + 
